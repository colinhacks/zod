...
Exception for `ctx.data` in connection with `z.coerce.date()`. If an invalid string is passed to `z.coerce.date()`, `ctx.data` is exceptionally `Invalid Date`. To circumvent this, use `z.string().datetime()` before `coerce` to validate the string before coercing it.
