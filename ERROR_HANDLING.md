...
Additionally, you can pass a mapping function of `(issue: ZodIssue) => U` to `flatMap()`, which is applied to the during the transformation of `ZodIssue`s.
...
```ts
console.log(err.flatMap( (i: ZodIssue) => { message: i.message, errorCode: i.code } ));
/*
  {
  formErrors: [],
  fieldErrors: {
