...
const CEILINGS: Record<string, number> = {
  ...
  // raised from 2974 / 3448 / 4561 by the commit that caused it: `parseAsync`/`safeParseAsync` walk synchronously first and remember a schema that hit async work, and `~standard.validate` shares that flag; `safeParseAsync` sits behind `~standard` in every bundle. Measured 3044 / 3517 / 4605 plus 28 headroom.
  "zod-mini-boolean": 3072,
  // Also carries the code-point string length scan: `.min`/`.max`/`.length` on a string pulls in the surrogate walk.
  "zod-mini-string": 3545,
