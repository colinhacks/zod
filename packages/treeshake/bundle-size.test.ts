...
const CEILINGS: Record<string, number> = {
  ...
  // raised from 2974 / 3448 / 4561 by the commit that caused it: `parseAsync`/`safeParseAsync` walk synchronously first and remember a schema that hit async work, `~standard.validate` shares that flag, and a declared-async callback throws before it runs — the predicate, the check flag and the guarded run land in every bundle. Measured 3129 / 3601 / 4682 plus 28 headroom.
  "zod-mini-boolean": 3157,
  // Also carries the code-point string length scan: `.min`/`.max`/`.length` on a string pulls in the surrogate walk.
  "zod-mini-string": 3629,
