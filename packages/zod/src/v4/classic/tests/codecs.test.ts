...
test("async operations", async () => {
  ...
  // Async transform
  const asyncTransform = z.string().transform(async (value: string) => {
...
test("pipe with transforms", () => {
  ...
  const pipeSchema = z.string().pipe(stringToNumber);
...
test("dangling transform detection", () => {
  ...
  const danglingPipe = stringWithChecks.pipe(danglingTransform);
...
test("complex pipes with multiple checks", () => {
  ...
  const complexPipe = stringWithLengthCheck.pipe(complexTransform).pipe(numberWithRangeCheck);
...
test("proper check execution order", () => {
  ...
  const edgeCasePipe = stringMin3.pipe(stringToLength).pipe(numberMin5);
