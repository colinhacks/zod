...
Sometimes the type comes first: a database model, a type from a generated client, an interface you don't own. Pass it to `z.schemaForType<T>()` and TypeScript checks that the schema's output type is exactly `T`.
...
const Player = z.schemaForType<Player>()(
  z.object({
    username: z.string(),
    xp: z.number(),
  })
...
z.schemaForType<Entry>()(z.object({ id: z.string() }).and(z.object({ label: z.string() }))); // ✅
z.schemaForType<Entry>()(z.object({ id: z.string() }).safeExtend({ label: z.string() })); // ❌
```

Flatten the target to accept either one:

```ts
type Flatten<T> = { [K in keyof T]: T[K] } & {};

z.schemaForType<Flatten<Entry>>()(z.object({ id: z.string() }).safeExtend({ label: z.string() })); // ✅
```
