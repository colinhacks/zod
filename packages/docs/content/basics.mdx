...
<Callout>
In most cases, you should define your schema first and infer the TypeScript type from it. If you need to write a schema against an externally defined type, you can use `satisfies` to check that the schema's output is assignable to it:
...
For Zod Mini, use `z.ZodMiniType<Player>` instead.

This keeps Zod's schema-first model while giving you a TypeScript check against an existing type. It is an assignability check, not an exact type equality check; extra object properties and omitted optional properties may still pass.
</Callout>
