...
| `z.array(z.object({…}))`, 10 items | ~6x |
| union of 3 objects | ~5.5x |
| tuple of 3 | ~3.5x |
...
Inside an object, array, tuple, record, or intersection, an unsupported child runs on the standard parser while the surrounding structure stays compiled. A union with an unsupported member, or any schema containing a `.catch()` callback, falls back as a whole.
