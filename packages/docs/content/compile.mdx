...
- **`ZodCompileUnsupportedError`** — the schema uses a construct the fast path can't reproduce exactly (e.g. `z.xor()`, checks with custom `when` conditions, or a recursive schema, which needs the standard parser to resolve reference cycles in the input).
