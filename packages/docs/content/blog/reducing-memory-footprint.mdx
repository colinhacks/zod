...
Zod 4.5 implements a "method memoization" pattern that allows it to defer allocating memory for bound methods until they are used (if ever). This post discussed that pattern.
...
By default all methods on Zod schemas are *auto-bound*, meaning they do not rely on implicit `this` semantics to work properly. That z-cenables patterns like this, which otherwise may break or behave in unexpected ways when reliant on unbound prototype methods.
...
This is now conclusively fixed in Zod 4.5 via a new pattern I call a "method memoization". 
...
| `z.object({})` | 6.6kb | 3.7kb | **1.8x** |
| `z.object({...})`, 3 keys | 22.2kb | 5.8kb | **3.8x** |
| `z.object({...})`, 10 keys | 82.0kb | 11.5kb | **7.1x** |
| `z.union([...])`, 2 options | 17.5kb | 2.1kb | **8.3x** |
| `z.discriminatedUnion(...)`, 2 options | 42.0kb | 13.7kb | **3.1x** |
| `z.lazy(...)` | 5.9kb | 1.3kb | **4.5x** |
| `zod/mini` `z.string()` | 2.5kb | 577b | **4.4x** |
| `zod/mini` `z.object({...})`, 3 keys | 11.8kb | 3.8kb | **3.1x** |
