...
Metadata in Zod is handled via *registries*. Registries are collections of schemas, each associated with some *arbitrary, strongly-typed* metadata. To create a simple registry:
...
> **Note** — This method is special in that it does not return a new schema; instead, it returns the original schema. No other Zod method does this (including `.meta` and `.describe`, documented below).
...
The `.describe()` method is a shorthand for registering a schema in the global registry with a `description` field.
