...
The `z.withExample()` utility function provides a convenient way to attach example values to schemas by encoding them as JSON in the schema's description field.
...
This is particularly useful for documentation generation, API specifications, and AI-powered tools that benefit from concrete examples. The example value is type-checked against the schema's inferred type.

```ts
const AssemblySchema = z.withExample(
  z.object({
...
// The example is stored in the description and can be retrieved
const example = JSON.parse(AssemblySchema.description!);
```
