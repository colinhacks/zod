...
The `.withExample()` method attaches example values to schemas via metadata. Examples are stored in an array, allowing you to provide multiple examples for the same schema.
...
This is particularly useful for documentation generation, API specifications, and AI-powered tools that benefit from concrete examples. The example value is **fully type-checked** against the schema's inferred type.

```ts
// Multiple examples can be chained
const ResponseSchema = z.object({
  status: z.enum(["success", "error"]),
  message: z.string(),
})
  .withExample({ status: "success", message: "Operation completed" })
...
Complex schemas that are merged, inherited, and unioned can be hard to understand at a glance. Examples make them immediately clear:

```ts
const AssemblySchema = CommandMessage.merge(Settings)
  .extend({
