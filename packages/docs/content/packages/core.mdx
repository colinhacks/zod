...
This sub-package exports the core classes and utilities that are consumed by `zod` and `zod/v4-mini`. It is not intended to be used directly; instead it's designed to be extended by other packages. It implements:
...
Refer to the implementations of `zod` and `zod/v4-mini` for examples of how to extend these classes. 
