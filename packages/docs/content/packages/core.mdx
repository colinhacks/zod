...
Both Zod and Zod Mini are user-facing "API frontends" over the shared validators in Zod Core. A regular Zod string schema must functionally extend both Zod Core's `$ZodString` class—which implements all string validation—as well as the `ZodType` class—defining Zod's base methods like `.optional()`, `.nullable()`, etc. This is the classic "diamond inheritance" problem.
...
Since JavaScript doesn't support multiple inheritance, Zod implements a *typesafe trait pattern*. Each "trait" is an interface coupled with an initializer function, created with `$constructor`.
...
The static `.init()` method applies the trait to a pre-existing object. Think of it like a constructor you can "apply" retroactively.
...
A trait can compose other traits by calling their `.init()` in its own initializer, instead of being restricted to a single `super()` call.
...
Each trait initializes at most once per instance—`Pet.init()` runs once for `Duck` even though both `Swimmer` and `Flyer` call it. `instanceof` checks `_zod.traits`, so `duck instanceof Pet` is `true`. This is the same diamond pattern `ZodString` uses: its initializer calls `$ZodString.init()` and `ZodType.init()`.
