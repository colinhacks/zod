...
The `zod/v4-mini` variant was introduced with the release of Zod 4. 
...
While `zod` automatically loads the English (`en`) locale, `zod/v4-mini` does not. This reduces the bundle size in scenarios where error messages are unnecessary, localized to a non-English language, or otherwise customized.
