...
Error maps also receive the schema associated with the issue as a second argument. This is useful for metadata-driven messages, especially for checks and refinements where `iss.inst` points to the check that originated the issue.

```ts
z.config({
  customError: (iss, schema) => {
    const label = schema ? z.globalRegistry.get(schema)?.title : undefined;
    return label ? `${label}: Invalid input` : undefined;
  },
});

const name = z.string().min(2).meta({ title: "Name" });
name.parse("A"); // => throws "Name: Invalid input"
```
