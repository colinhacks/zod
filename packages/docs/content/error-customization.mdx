...
For convenience, all locales are also exported as `z.locales`, so `z.config(z.locales.de())` works without the extra import. Rollup and Webpack tree-shake that down to the locales you actually use. esbuild ([evanw/esbuild#1420](https://github.com/evanw/esbuild/issues/1420)) and Turbopack bundle every locale, whichever import form you use, so stick to `zod/locales` if you bundle with either.
