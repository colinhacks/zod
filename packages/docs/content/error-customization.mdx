...
Import locales directly from `zod/v4/locales/*` for guaranteed tree-shaking:
...
Import locales directly from their paths to ensure only the locales you need are bundled:

```ts
import * as z from "zod/v4";
import de from "zod/v4/locales/de";
import fr from "zod/v4/locales/fr";
import ja from "zod/v4/locales/ja";
...
**Alternative:** You can also use named imports (should be tree-shakeable in modern bundlers):
```ts
