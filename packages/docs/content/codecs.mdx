...
Converts integer enums to strings. Both the strings and integers are type safe.

```ts
const fruits = ['Apples', 'Bananas', 'Oranges', 'Peaches', 'Pears'] as const;
...
export const fruitEnumToString = z.codec(
    z.union(fruits.map((_r, i) => z.literal(i as FruitIndex))), // what's in the source data
...
fruitEnumToString.decode(10); // ZodError + TypeScript error
fruitEnumToString.decode('Potato') // ZodError + TypeScript error

fruitEnumToString.encode('Apples'); // => 0;
fruitEnumToString.encode('Potato'); // ZodError + TypeScript error
fruitEnumToString.encode(1); // ZodError + TypeScript error
```
