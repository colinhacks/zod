...
All Zod schemas (with some specific exceptions described below) are "codecs". More specifically, they can process inputs in both the forward ("decoding"/"parsing") and backward ("encoding") directions. You probably already know how to parse data with Zod:
