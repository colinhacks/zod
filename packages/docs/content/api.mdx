...
The `z.iso.duration()` method validates ISO 8601 duration strings:

```ts
const duration = z.iso.duration();

duration.parse("P3Y6M4DT12H30M5S"); // ✅ (3 years, 6 months, 4 days, 12 hours, 30 minutes, 5 seconds)
duration.parse("PT12H30M5S"); // ✅ (12 hours, 30 minutes, 5 seconds)
duration.parse("P1Y"); // ✅ (1 year)
duration.parse("PT6H"); // ✅ (6 hours)
duration.parse("P5W"); // ✅ (5 weeks)
duration.parse("P2Y9M3DT12H31M8.001S"); // ✅ (with milliseconds)
duration.parse("PT0,001S"); // ✅ (comma as decimal separator)
duration.parse("foo bar"); // ❌
duration.parse("P"); // ❌ (incomplete)
duration.parse("T1H"); // ❌ (missing P prefix)
```

The format starts with `P` (for period) followed by date components (`Y` years, `M` months, `D` days), then optionally `T` (for time) followed by time components (`H` hours, `M` minutes, `S` seconds).
