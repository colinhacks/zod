...
While `.partial()` only touches the top-level shape, `z.deepPartial()` recurses through arrays, tuples, unions, records, and wrapper types to reach every object in the tree.
...
const PartialRecipe = z.deepPartial(Recipe);

PartialRecipe.parse({ author: {}, ingredients: [{}] }); // ✅
PartialRecipe.parse({}); // ✅
```

The result is still a `ZodObject`, so the object APIs keep working:

```ts
PartialRecipe.shape.title; // ZodOptional<ZodString>
PartialRecipe.extend({ servings: z.number() });
```

[Recursive objects](#recursive-objects) work too:
...
z.deepPartial(Category).parse({ subcategories: [{ subcategories: [{}] }] }); // ✅
```

The input schema is never modified. A [discriminated union](#discriminated-unions) degrades to a plain `z.union()`, since an optional discriminator defeats the discriminated lookup.

`z.deepPartial()` inherits `.partial()`'s restriction on refinements, and reaches deeper for it — an object anywhere in the tree carrying its own [refinement](#refinements) throws:

```ts
z.deepPartial(z.object({ a: z.object({ b: z.string() }).refine((v) => !!v.b) }));
// ❌ .partial() cannot be used on object schemas containing refinements
```

`z.deepPartial()` is a single call to [`z.visit()`](#visit); reach for that directly to write other deep rewrites.
...
These are the runtime counterparts to the type-level `z.input<T>` and `z.output<T>`. They walk a schema and replace every pipe or [codec](/codecs) node with its input or output side.
...
A codec's `.in` and `.out` only reach a codec you hold directly. These reach codecs nested anywhere in the tree — inside objects, arrays, records, and maps:

```ts
const Prices = z.record(z.string(), isoDate);

z.input(Prices).parse({ launch: "2024-01-01T00:00:00Z" }); // ✅
```

Sharing a name with the type-level helpers is deliberate — TypeScript resolves types and values in separate namespaces:

```ts
type EventIn = z.input<typeof Event>; // { name: string; at: string }
const EventIn = z.input(Event);       // ZodType<{ name: string; at: string }>
```

A schema with no pipes is returned as-is, not cloned.

These project the two *sides* of a pipe, so they are meant for [codecs](/codecs), where both sides are real schemas. A one-way [transform](#transforms) has no output schema to project to — `z.output()` hands back the transform itself, which validates nothing:

```ts
z.output(z.string().transform((v) => v.length)).parse(123); // => undefined, no error
```
...
<Callout type="warn">
  **Experimental** — The `z.visit()` function is experimental and is not considered part of Zod's stable API. Its traversal contract is likely to change in future releases.
</Callout>

`z.visit()` rewrites a schema tree from the bottom up. Each node is visited after its children, and whatever the visitor returns takes its place. It's the primitive behind [`z.deepPartial()`](#deep-partial) and [`z.input()` / `z.output()`](#input-output).

Pass a handler map keyed by `def.type` to target one kind of node:

```ts
function deepStrict<T extends z.core.SomeType>(schema: T): T {
  return z.visit(schema, { object: (o) => (o as z.ZodObject).strict() });
}

const Strict = deepStrict(z.object({ a: z.object({ b: z.string() }) }));
Strict.parse({ a: { b: "hi", extra: 1 } }); // ❌ throws
```

`deepReadonly` is the same shape:

```ts
function deepReadonly<T extends z.core.SomeType>(schema: T): T {
  return z.visit(schema, { object: (o) => (o as z.ZodObject).readonly() });
}
```

Kinds with no handler pass through untouched. That includes user-defined `def.type`s outside Zod's built-in vocabulary — to intercept those, pass a callback instead of a map and dispatch yourself:

```ts
const rewritten = z.visit(schema, (node) =>
  node._zod.def.type === "money" ? myMoneyRewrite(node) : node
);
```

Nodes are re-cloned only when one of their children actually changed, so an identity visit gives back the schema you passed in:

```ts
const Recipe = z.object({ title: z.string() });

z.visit(Recipe, (node) => node) === Recipe; // => true
```

Shared sub-schemas are visited once. Cycles terminate: `z.lazy()` nodes defer traversal to parse time, and [getter-based recursion](#recursive-objects) is broken with a lazy placeholder. Both of those cases always re-clone, so they don't preserve identity.

`z.visit()` returns the same type it was given — it does not infer the rewrite. When a rewrite changes the inferred type, declare that yourself, the way `z.deepPartial()` declares its own `DeepPartial<T>`.
