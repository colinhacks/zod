...
This makes it possible to safely extend schemas that contain refinements. (Regular `.extend()` will throw an error when used on schemas with refinements.)
...
const UserSchema = z.object({
  name: z.string(),
  age: z.number()
}).refine(user => user.age >= 18, {
  message: "User must be at least 18 years old"
});

// ❌ This would throw an error:
// const Extended = UserSchema.extend({ email: z.string() });

// ✅ This works:
const ExtendedUser = UserSchema.safeExtend({
...
// ❌ This would throw an error:
// const Extended = z.extend(UserSchema, { email: z.string() });

// ✅ This works:
const ExtendedUser = z.safeExtend(UserSchema, {
  email: z.string().email()
});
```
</Tab>
...
When using `.safeExtend()`, the original schema's refinements are preserved in the extended schema:

```ts
const result = ExtendedUser.parse({
  name: "John",
  age: 16, // This will fail the refinement
  email: "john@example.com"
});
// ZodError: User must be at least 18 years old
```

<Callout type="info">
**Type safety**: `.safeExtend()` provides compile-time type checking to prevent you from overriding existing fields with incompatible types. If you try to extend a schema with a field that already exists but has a different type, TypeScript will show an error.

```ts
const User = z.object({ name: z.string() });

// ❌ TypeScript error - can't override string with number
User.safeExtend({ name: z.number() });

// ✅ This works - same type
User.safeExtend({ name: z.string().min(2) });
```
</Callout>
