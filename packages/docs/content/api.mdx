...
Use `z.properties()` to check several properties at once. It returns an array of `z.property()` checks to spread into `.check()`. Chained `z.property()` checks stop at the first failure; each check from `z.properties()` runs regardless, so one parse reports every property that didn't match.
...
httpsUrl.parse(new URL("http://localhost")); // ❌ reports protocol and hostname
```
