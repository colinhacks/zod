...
```ts
const blobSchema = z.instanceof(URL).properties({
  protocol: z.literal("https:" as string, "Only HTTPS allowed"),
...
blobSchema.parse(new URL("https://localhost")); // ❌
```

The top-level `z.properties()` API generalizes `z.property()` to validate multiple properties at once.
...
httpsUrl.parse(new URL("http://example.com")); // ❌
httpsUrl.parse(new URL("https://localhost")); // ❌
```
