...
<Callout>
Coercion allows providing the expected input type,
```ts
const MySchema = z.object({
  coerced: z.coerce.number<number>().optional(),
})
```
This is useful when using a standard schema based form validator to avoid errors such as 
</Callout>
