...
Validate an International Bank Account Number (IBAN): 2-letter country code, 2 check digits, and 11 to 30 alphanumeric characters with a valid [ISO 7064 MOD 97-10](https://en.wikipedia.org/wiki/International_Bank_Account_Number#Validating_the_IBAN) checksum. Electronic format only (no spaces or hyphens).

```ts
const iban = z.iban();
iban.parse("DE89370400440532013000"); // ✅
iban.parse("NO9386011117947");        // ✅
iban.parse("DE89 3704 0044 0532 01"); // ❌ no spaces
