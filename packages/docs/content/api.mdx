...
<Callout type="warn">
  When merging object schemas, prefer [`A.extend(B)`](#extend) over intersections. Using `.extend()` will gve you a new object schema, whereas `z.intersection(A, B)` returns a `ZodIntersection` instance which lacks common object methods like `pick` and `omit`.
</Callout>
...
### Strict branded types

By default, branded types only affect the *output* type of a schema - the `input` type remains unbranded. This means you can pass plain data to the schema's `.parse()` method, but the result will be branded.

```ts
const UserId = z.string().brand<"UserId">();

type UserIdInput = z.input<typeof UserId>;   // string (unbranded)
type UserIdOutput = z.output<typeof UserId>; // string & z.$brand<"UserId"> (branded)

UserId.parse("abc123"); // returns branded string
```

In some cases, you may want the input type to also be branded. To enable this, use the `strict` option:

```ts
const StrictUserId = z.string().brand<"UserId", { strict: true }>();
// or alternatively:
const StrictUserId = z.string().brand("UserId", { strict: true });

type StrictUserIdInput = z.input<typeof StrictUserId>;   // string & z.$brand<"UserId"> (branded)
type StrictUserIdOutput = z.output<typeof StrictUserId>; // string & z.$brand<"UserId"> (branded)
```

With strict branded types, both the input and output types are branded, meaning you can only pass already-branded data to the schema.
