...
<Callout>
  The `error` param also accepts a function, which receives the issue context and can return an input-aware message. See [Customizing errors](/error-customization#the-error-param).
</Callout>
...
Use `.refine()` for simple boolean checks and `.superRefine()` when you need to emit multiple issues or control issue codes. Both can be chained on the same schema, and all checks run in order.
...
const Password = z
  ...
  .superRefine((val, ctx) => {
    if (!/[A-Z]/.test(val)) {
      ctx.addIssue({ code: "custom", message: "Must contain an uppercase letter.", input: val });
    }
    if (!/[0-9]/.test(val)) {
      ctx.addIssue({ code: "custom", message: "Must contain a number.", input: val });
    }
  });
...
/* [
  { "code": "custom", "message": "Must contain an uppercase letter." },
  { "code": "custom", "message": "Must contain a number." }
] */
```
</Tab>
<Tab value="Zod Mini">
...
const Password = z.string().check(
  z.refine((val) => val.length >= 8, { error: "Must be at least 8 characters.", abort: true }),
  z.superRefine((val, ctx) => {
    if (!/[A-Z]/.test(val)) {
      ctx.addIssue({ code: "custom", message: "Must contain an uppercase letter.", input: val });
    }
...
const result = z.safeParse(Password, "hello");
result.error?.issues;
/* [
  { "code": "custom", "message": "Must be at least 8 characters." }
] */

const result2 = z.safeParse(Password, "hello world");
result2.error?.issues;
/* [
  { "code": "custom", "message": "Must contain an uppercase letter." },
  { "code": "custom", "message": "Must contain a number." }
...
The first refinement is marked `abort: true`, so its failure terminates validation before `.superRefine()` runs.
