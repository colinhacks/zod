...
import { Tabs as CodeGroup } from "fumadocs-ui/components/tabs"
...
z.email(z.regexes.emai);

// the regex used by browsers to validate input[type=email] fields
// https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input/email
z.email(z.regexes.html5Email);

// the classic emailregex.com regex (RFC 5322)
z.email(z.regexes.rfc5322Email);

// a loose regex that allows Unicode (good for intl emails)
z.email(z.regexes.unicodeEmail);
```
...
ipv4.parse("84d5:51a0:9114:1855:4cfa:f2d7:1f12:7003"); // ❌

const ipv6 = z.string().ip({ version: "v6" });
ipv6.parse("192.168.1.1"); // ❌
```

To allow both IPv4 and IPv6:

```ts
const ip = z.string().ip();

ip.parse("192.168.1.1"); // ✅
ip.parse("84d5:51a0:9114:1855:4cfa:f2d7:1f12:7003"); // ✅
ip.parse("84d5:51a0:9114:1855:4cfa:f2d7:1f12:192.168.1.1"); // ✅

ip.parse("256.1.1.1"); // ❌
ip.parse("84d5:51a0:9114:gggg:4cfa:f2d7:1f12:7003"); // ❌
```
...
ipv4Cidr.parse("84d5:51a0:9114:1855:4cfa:f2d7:1f12:7003"); // ❌

const ipv6Cidr = z.string().cidr({ version: "v6" });
ipv6Cidr.parse("192.168.1.1"); // ❌
```
...
The regular `.refine` API only generated create issues with a `"custom"` error code, but `.superRefine()` makes it possible to throw other issue types. For more information on Zod's internal issue types, read the [Error customization]`(/error-customization) docs.
...
Since piping a schema into a transform is such a common pattern, Zod provides a convenience `.transform()` method.
...
<Callout>
  ...
  <Tab value="zod">
  ```ts
  const result = await idToUser.parseAsync("abc123");
  ```
  </Tab>
  <Tab value="@zod/mini">
  ```ts
  const result = await z.parseAsync(idToUser, "abc123");
  ```
  </Tab>
  </Tabs>
</Callout>
...
<Accordions>
<Accordion title="Validating inside transform">

To perform validation logic inside a transform, use `ctx`. To report a validation issue, push a new issue onto `ctx.issues`. This is the same as the [`.check()`](#check) API.
