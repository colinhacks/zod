...
<Accordions>
  <Accordion title="Resolving type errors in recursive schemas">
    ...
    const Activity = z.object({
      name: z.string(),
      get children() {
        return z.optional(z.array(Activity)); // ❌
        return Activity.array().optional() // ✅
      },
    });
    ```
...
<Accordions>
<Accordion title="Partial records">
  If you want a *partial* record type, you'll need to trick Zod a little. This short-circuits the special handling of `z.enum()`.
