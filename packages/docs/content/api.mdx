...
z.cuid2();
z.ulid();
z.objectId();     // MongoDB ObjectId (24 hex chars)
z.ipv4();
z.ipv6();
...
To validate MongoDB ObjectId strings (24 hexadecimal characters):

```ts
z.objectId();
```

This accepts both lowercase and uppercase hex. For custom needs, you can use the underlying regex directly:

```ts
z.string().regex(z.regexes.objectid);
```
