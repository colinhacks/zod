...
Libraries should *only* be using types from `zod/v4/core`. This sub-package defines the interfaces, classes, and utilities that are shared between `zod/v4` and `zod/v4-mini`. By building against the shared base interfaces, you can reliably support both sub-packages without duplicate code.
...
This function can accept both `zod/v4` and `zod/v4-mini` schemas. 
