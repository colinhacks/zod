...
Many utility functions and types have been moved to the new `zod/v4/core` package, to facilitate code sharing between `zod` and `zod/v4-mini`. The contents of `zod/v4/core` from `zod`/`zod/v4-mini` using the `z.core` namespace. Check `z.core` if any internal APIs you rely on are missing; they've likely been moved there.
...
A new `zod/v4-mini` package is being released concurrently with Zod 4. This is a new package optimized for tree-shakability and intended for use in bundle-size-constrained applications. 

To facilitate code sharing, both `zod` and `zod/v4-mini` have a dependency on a new common core library called `zod/v4/core`. This library implements the base classes of all schemas (`$ZodType` base class, plus `$ZodString`, `$ZodNumber`, etc), utility functions/types, issue types (`$ZodIssueInvalidType`, etc.), and the base error class (`$ZodError`).

As you can see these things are all prefixed with `$` to indicate they are implemented in `zod/v4/core`. Many things formerly exported from `zod` have been moved to `zod/v4/core`. You can access the contents of `zod/v4/core` from `zod`/`zod/v4-mini` using the `z.core` namespace.
