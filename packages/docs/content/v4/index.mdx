...
That's where `@zod/mini` comes in. It's a sister library with functional, tree-shakable API that corresponds one-to-one with `zod`. Use the tabs to compare the two APIs.
