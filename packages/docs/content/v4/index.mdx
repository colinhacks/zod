...
The core bundle is ~57% smaller in Zod 4. That's good!

But ultimately Zod's method-heavy API is fundamentally difficult to tree-shake. Even our simple `z.boolean()` schema pulls in the implementations of a bunch of methods we didn't use, like `.optional()`, `.array()`, etc. Writing slimmer implementations can only get you so far. 
...
When we build this with `rollup`, the gzipped bundle size is `1.88kb`. That's a 6.6x reduction in core bundle size compared to `zod@3`.
...
> The `@zod/mini` package doesn't have its own documentation. Instead, all code examples in the [Defining schemas](/api) now have separate code tabs for `zod` and `@zod/mini`.
