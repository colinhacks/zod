...
<tbody>
  ...
  <tr>
    ...
    <td>
      ...
      - `"ref-registry"` — All schemas with `id` in metadata will be extracted as `$defs`
    </td>
...
> With `reused` option set to `"ref-registry"` it is possible to only extract schemas with an `id` property (reused schemas without `id` will be inlined) into `$defs`. 
