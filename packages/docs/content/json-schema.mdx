...
Optional schemas are represented as-is. Optionality only changes the JSON Schema when the schema is nested inside an object.
...
interface ToJSONSchemaParams {
  ...
  /** A function used to convert `id` values to URIs to be used in *external* $refs. Only used when generating JSON Schema from a registry.
   *
   * Default is `(id) => id`.
   */
...
All schemas with an `id` property will be extracted into `$defs`/`definitions`:
