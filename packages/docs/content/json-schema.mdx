---
title: "JSON Schema"
description: "How to convert between Zod schemas and JSON Schema"
---
...
To validate data with an existing JSON Schema, use `z.jsonSchema()`. This creates a Zod schema that delegates validation to [AJV](https://ajv.js.org/).
...
User.parse({ name: "Colin", age: 30 }); // => { name: "Colin", age: 30 }
User.parse({ name: "Colin" }); // throws ZodError
```

You can also configure an AJV instance globally:
...
When both are provided, the local `{ ajv }` option takes precedence. Compiled validators are cached by AJV instance and JSON Schema object identity, so reuse schema objects in performance-sensitive paths.
