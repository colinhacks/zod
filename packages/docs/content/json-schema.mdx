...
To decide case by case, pass a function instead. It's called once per unrepresentable schema. Return a JSON Schema to use in its place, `"any"`, or `"throw"` (the default when you return nothing).
...
The handler also receives the `path` of the schema, and any error it throws propagates, so you can report unrepresentable types in your own words.

```ts
z.toJSONSchema(schema, {
  unrepresentable: ({ zodSchema, path }) => {
    throw new Error(`${zodSchema._zod.def.type} at /${path.join("/")} is not supported`);
  },
});
```
