          ...
          return result.then(({ issues, value }) => {
            ...
            return {
              issues: issues.map((iss) =>
                util.finalizeIssue(iss, {}, core.config())
              ),
            };
          });
        }
...
export const $ZodString: core.$constructor<$ZodString> =
  /*@__PURE__*/ core.$constructor("$ZodString", (inst, def) => {
    ...
    inst._zod.pattern =
      inst?._zod.computed?.pattern ?? regexes.string(inst._zod.computed);
...
export interface $ZodUnionInternals<
  T extends readonly $ZodType[] = readonly $ZodType[],
...
export const $ZodDiscriminatedUnion: core.$constructor<$ZodDiscriminatedUnion> =
  ...
  core.$constructor("$ZodDiscriminatedUnion", (inst, def) => {
    ...
    for (const el of def.options) {
      if (!el._zod.disc)
        throw new Error(
          `Invalid discriminated union option at index "${def.options.indexOf(el)}"`
        );
    ...
    for (const option of def.options) {
      ...
      if (!disc) {
        throw new Error(
          `Invalid disciminated union element: ${option._zod.def.type}`
        );
      }
    ...
    inst._zod.parse = (payload, ctx) => {
      ...
      if (filteredOptions.length === 1)
        return filteredOptions[0]._zod.run(payload, ctx) as any;
...
export const $ZodTuple: core.$constructor<$ZodTuple> =
  /*@__PURE__*/ core.$constructor("$ZodTuple", (inst, def) => {
    ...
    const optStart =
      ...
      [...items]
        .reverse()
        .findIndex((item) => item._zod.optionality !== "optional");
...
export const $ZodNullable: core.$constructor<$ZodNullable> =
  /*@__PURE__*/ core.$constructor("$ZodNullable", (inst, def) => {
    ...
    if (pattern)
      inst._zod.pattern = new RegExp(
        `^(${util.cleanRegex(pattern.source)}|null)$`
      );
...
////////////////////////////////////////////
////////////////////////////////////////////
export interface $ZodDefaultDef<T extends $ZodType = $ZodType>
  extends $ZodTypeDef {
  type: "default";
  innerType: T;
...
///////////////////////////////////////////////
///////////////////////////////////////////////
export interface $ZodNonOptionalDef<T extends $ZodType = $ZodType>
  extends $ZodTypeDef {
  type: "nonoptional";
  innerType: T;
}
...
export interface $ZodNonOptional<T extends $ZodType = $ZodType>
  extends $ZodType {
  ...
  /*@__PURE__*/ core.$constructor("$ZodNonOptional", (inst, def) => {
    ...
    if (def.innerType._zod.values)
      inst._zod.values = new Set(
        [...def.innerType._zod.values].filter((x) => x !== undefined)
      );
    inst._zod.parse = (payload, ctx) => {
      const result = def.innerType._zod.run(payload, ctx);
      if (result instanceof Promise) {
...
export const $ZodLazy: core.$constructor<$ZodLazy> =
  /*@__PURE__*/ core.$constructor("$ZodLazy", (inst, def) => {
    ...
    util.defineLazy(
      inst._zod,
      "optionality",
      () => inst._zod._getter._zod.optionality
    );
    inst._zod.parse = (payload, ctx) => {
