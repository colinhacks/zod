...
| [#5178](https://github.com/colinhacks/zod/issues/5178) / [#6400](https://github.com/colinhacks/zod/pull/6400) | Made `$ZodObject` key omission read `optout`, so a property that is optional in and required out — every `.transform()` — emits the key its inferred type promises. Declined the inverse fix ([#6382](https://github.com/colinhacks/zod/pull/6382)), which gave `$ZodTransform` an `optout` and restored the v3 key-optional type. Also fixed the JIT fastpass keeping a value written alongside a swallowed issue. |
