...
z.object({ a: z.string().optional().transform((s) => s?.trim()) }).parse({})
// → { a: undefined }  with the key PRESENT  (pipe.optout = transform.optout = undefined, so
//   `a` is a required key in z.output; obj writes it even though the input omitted it)
//   pre-#6400: was {} — the key the inferred type promised was silently missing
...
| [#5178](https://github.com/colinhacks/zod/issues/5178) / [#6400](https://github.com/colinhacks/zod/pull/6400) | Made `$ZodObject` key omission read `optout`, so a property that is optional in and required out emits the key its inferred type promises. That cell is where an `optin`-optional schema piped into a `.transform()` lands, since the transform erases `optout`. Declined the inverse fix ([#6382](https://github.com/colinhacks/zod/pull/6382)), which gave `$ZodTransform` an `optout` and restored the v3 key-optional type. Also fixed the JIT fastpass keeping a value written alongside a swallowed issue. |
