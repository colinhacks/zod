...
- **String formats with custom runtime behavior** — done. `url`/`httpurl`/URL options use exported `parseValidURL` via a hoisted helper; `base64`, `base64url`, `jwt`, `ipv6`, `cidrv6` and `credit_card` use hoisted runtime validators; a custom format compiles `def.fn`. The regex path is now gated on an allowlist of formats whose pattern is the whole check, so a format that layers extra validation over a shape-only pattern can no longer compile to the pattern alone. A format in neither list force-falls-back.
- **Record key schemas** — done. A key schema with no value set (`z.email()`, `z.string().min(2)`, `z.number()`, a template literal, a key transform) compiles once and runs per key, in strict and loose mode alike. The bare-`z.string()` shortcut now also excludes string formats (which live on the def, not in `checks`) and coerced keys, both of which it previously accepted unchecked. Remaining record fallbacks: a `__proto__` key value.
- **Recursive schemas** — force-fallback. A subtree containing a cycle can be re-entered by one parse, which is how the memoizer terminates input with a reference cycle; its state is keyed on the parse context, and generated code has none. Lifting this means threading the ctx and `memo.alloc` into compiled containers, at which point the back-edge bail in `compile()`'s wrapper can go too.
