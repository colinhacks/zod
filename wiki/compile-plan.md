...
- **exactOptional** — done for top-level schemas and object properties. Top-level delegates to the inner schema so `undefined` rejects; object codegen observes `key in input` and only invokes the child when present, matching runtime. Tuple exactOptional remains a separate output-shaping case.
