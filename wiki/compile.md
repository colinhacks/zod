...
- **No async.** The compiler eagerly throws if it encounters an async refinement, transform, or check during the codegen walk. There is no affordance for promises anywhere in generated code. `parseAsync` and `safeParseAsync` walk synchronously first, so a schema with no async work still takes the fast path; a schema that has hit async work is flagged and goes straight to the runtime from then on.
