...
type MapToReadonlyDeep<T> = T extends []
  ? []
  : T extends readonly [z.ZodReadonly<infer V>, ...infer Tail]
  ? [z.ZodReadonly<V, "deep">, ...MapToReadonlyDeep<Tail>]
  : never;

const deepReadonlySchemas_0 = schemas.map((s) => s.deep()) as MapToReadonlyDeep<
  typeof schemas
>;
      ...
      .set(
        z
          .promise(
            z
              .map(
                z.record(z.string(), z.array(z.any()).readonly()).readonly(),
                z.tuple([z.string()]).readonly().readonly()
              )
              .readonly()
          )
          .readonly()
...
const crazyDeepReadonlySchema = z
  .map(
    z.set(z.tuple([z.string(), z.number()])),
    z.object({
      a: z.record(z.string(), z.array(z.any())),
      b: z.object({
        c: z.object({
          d: z.object({
            e: z.object({
...
test("deep inference", () => {
  ...
  util.assertEqual<z.infer<typeof deepReadonlySchemas_0[9]>, Readonly<unknown>>(
    true
  );
  ...
  util.assertEqual<
    ...
    Readonly<testEnum>
  >(true);
  util.assertEqual<z.infer<typeof deepReadonlySchemas_0[20]>, Promise<string>>(
    true
  );
