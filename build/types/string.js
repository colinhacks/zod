...
class ZodString extends z.ZodType {
    constructor() {
        super(...arguments);
        ...
        this.min = (minLength, msg) => this.refine((data) => data.length >= minLength, msg || `Value must be ${minLength} or more characters long`);
        this.max = (maxLength, msg) => this.refine((data) => data.length <= maxLength, msg || `Value must be ${maxLength} or fewer characters long`);
        this.length = (len, msg) => this.refine((data) => data.length === len, msg || `Value must be ${len} characters long.`);
        this.email = (msg) => this.refine((data) => emailRegex.test(data), msg || 'Invalid email address.');
        this.url = (msg) => this.refine((data) => urlRegex.test(data), msg || 'Invalid URL.');
        this.nonempty = (msg) => this.refine((data) => data.length > 0, msg || 'Value cannot be empty string');
    }
}
